...
def init_centroids(num_clusters, image):
    ...
    for i in range(num_clusters):
        randR = random.randint(0, image.shape[0] - 1)
        randC = random.randint(0, image.shape[1] - 1)
        centroids_init[i] = image[randR][randC]
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    while n < max_iter:
        ...
        centroid_pix = {x: [] for x in range(num_centroids)}
        ...
        for i in range(row):
            ...
            for j in range(col):
                ...
                for k, centroid in enumerate(new_centroids):
                    ...
                    pixel = image[i][j]
                    d = np.sqrt((pixel[0] - centroid[0])**2 + (pixel[1] - centroid[1])**2 + (pixel[2] - centroid[2])**2)
                    dist.append([k, d])
                    
                closest = min(dist, key = lambda x: x[1])[0]
                
                centroid_pix[closest].append(image[i][j])

        for i, pix in centroid_pix.items():
            
            valueSum = np.sum(pix, axis = 0)

            for j in range(3):
                
                if len(pix) != 0:
                    new_centroids[i][j] = int(valueSum[j]/len(pix))
                else:
                    new_centroids[i][j] = 0

        n += 1

        if np.linalg.norm(new_centroids - old_centroids) == 0:
            break
...
def update_image(image, centroids):
    ...
    ----------
    image : nparray
        (H, W, C) image represented as an nparray2
    centroids : int
        The centroids stored as an nparray
    ...
    for i in range(r):
        for j in range(c):
            ...
            for k, cen in enumerate(centroids):
                pixel = image[i][j]
                dist.append(np.sqrt((pixel[0] - cen[0])**2 + (pixel[1] - cen[1])**2 + (pixel[2] - cen[2])**2))
                minDist = list(dist).index(min(list(dist)))
            image[i][j] = centroids[minDist]
