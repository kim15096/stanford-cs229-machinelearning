...
"""
Parts of the code (cart and pole dynamics, and the
state
...
def initialize_mdp_data(num_states):
    ...
    # *** START CODE HERE ***
    transitionsCount = np.zeros([num_states,num_states,2])
    rewardsCount = np.zeros([num_states,2])
    rewards = np.zeros(num_states)
    tProbs = np.ones([num_states, num_states, 2])/num_states
    valueFunc = np.random.rand(num_states)*0.1
    dic_MDP = {'tc': transitionsCount,
               'rc': rewardsCount, 'rewards': rewards,
               'tProbs':tProbs,'value':valueFunc,'number': num_states}
...
def choose_action(state, mdp_data):
    ...
    dotProduct = mdp_data['tProbs'][state,:,0].dot(mdp_data['value'])
    dotProduct1 = mdp_data['tProbs'][state,:,1].dot(mdp_data['value'])

    if dotProduct > dotProduct1:
        return 0

    elif dotProduct < dotProduct1:
        return 1
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    mdp_data['tc'][state,new_state,action] += 1
    mdp_data['rc'][new_state, 0] += reward
    mdp_data['rc'][new_state, 1] += 1
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for action in [0,1]:
        for state in range(mdp_data['number']):
            total = np.sum(mdp_data['tc'][state,:,action])
            
            if total > 0:
                mdp_data['tProbs'][state,:,action] = mdp_data['tc'][state,:,action]/total

    for state in range(mdp_data['number']):
        if mdp_data['rc'][state, 1] > 0:
            mdp_data['rewards'][state] = mdp_data['rc'][state,0]/mdp_data['rc'][state,1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while max(abs(prevVal - mdp_data['value']) > tolerance):
              
        prevVal = mdp_data['value'].copy()
        newVal = np.zeros(n)
        
        for state in range(n):
            dot = gamma*mdp_data['tProbs'][state,:,0].dot(mdp_data['value'])
            dot1 = gamma*mdp_data['tProbs'][state,:,1].dot(mdp_data['value'])

            newVal[state] = max(dot, dot1)

        newVal = mdp_data['rewards'] + newVal

        mdp_data['value'] = newVal
